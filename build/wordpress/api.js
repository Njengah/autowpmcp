  ...
  try {
    ...
    if (wpConfig.appPassword) {
      const credentials = Buffer.from(
        `${wpConfig.username}:${wpConfig.appPassword}`
      ).toString("base64");
    ...
    if (!wpConfig.password) {
      throw new Error(
        "No password or app password provided for WordPress authentication"
      );
    }
    const credentials = Buffer.from(
    ...
    throw new Error(
      "Failed to create authentication header: " +
...
export async function createPost(
  ...
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? formatErrorResponse(error) : "Unknown error",
    };
  }
}
